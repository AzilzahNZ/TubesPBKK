...
@section('content')
    <div class="card card-body" style="max-width: 100%; margin: 0 auto; padding: 10px;">
        ...
        <table class="table table-bordered" style="max-width: 100%;">
            ...
            @if ($riwayat_pengajuan_surats->jenis_surat === 'Proposal Permohonan Dana')
                <tr>
                    <th>Nominal Dana yang Diajukan</th>
                    <td>{{ $riwayat_pengajuan_surats->nominal_dana !== null ? 'Rp ' . number_format($riwayat_pengajuan_surats->nominal_dana, 0, ',', '.') : '-' }}
                    </td>
                </tr>
                <tr>
                    <th>Nominal Dana yang Disetujui</th>
                    <td>{{ $riwayat_pengajuan_surats->nominal_dana_disetujui !== null ? 'Rp ' . number_format($riwayat_pengajuan_surats->nominal_dana_disetujui, 0, ',', '.') : '-' }}
                    </td>
                </tr>
            @endif
            <tr>
